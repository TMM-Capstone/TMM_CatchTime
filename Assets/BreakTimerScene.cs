...
public class BreakTimerScene : MonoBehaviour
{
    public TextMeshProUGUI timerText; // TMPro를 사용한 Text
    public Image filledImage; // Filled 이미지의 Image 컴포넌트
    public TextMeshProUGUI fadeInOutText; // Fade in/out 할 텍스트

    private int breakTimeInSeconds; // 초 단위로 계산할 변수 추가
    ...
    void Start()
    {
        breakTimeInSeconds = PlayerPrefs.GetInt("BreakTime"); // 사용자로부터 초 단위의 휴식 시간을 입력 받음

        // 초기에 filled 이미지를 숨기기 위해 fillAmount를 0으로 설정
        filledImage.fillAmount = 0f;

        StartCoroutine(StartBreakTimer());
        StartCoroutine(FadeInOutText());
    }

    IEnumerator StartBreakTimer()
    {
        while (breakTimeInSeconds >= 0) // breakTimeInSeconds이 0 이상일 때까지 반복
        {
            timerText.text = FormatTime(breakTimeInSeconds); // 텍스트로 표시
            float fillTarget = 1f - ((float)breakTimeInSeconds / (float)PlayerPrefs.GetInt("BreakTime")); // 채워질 목표량 계산

            // filled 이미지를 숨기고 나서 채우기 시작
            StartCoroutine(FillImage(fillTarget));

            // 타이머가 00:01이 되었을 때 텍스트 Fade Out 시작
            if (breakTimeInSeconds == 1 && fadeInTextFinished)
            {
        ...
        SceneManager.LoadScene("NewRewardScene");
    }
